...
def extract_imports(filepath):
    try:
        walked = ast.walk(ast.parse(filepath.read_text(), filepath.name))
...
def ast_module_loader(module, collector):
    ...
    for node_cls, node, absolute_path in extract_imports(module):
        if node_cls == 'ImportFrom':
            ...
            if absolute_path:
                node_module = '.'.join((absolute_path, '')).strip('.')
...
def extract_all_imports(paths=None):
    ...
    filenames = (filename for path in paths for filename in ([path] if path.is_file() else path.rglob('*.py')))

    for idx, module in enumerate(filenames, start=1):
        ast_module_loader(module, collector)
    print('.py files processed:', idx)
