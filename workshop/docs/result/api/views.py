...
class MyJsonResponse(JsonResponse):
    def __init__(
        self, request=None, template=None, context=None, using=None, **response_kwargs
    ):
...
class AsyncPetDetailView(BaseDetailView):
    ...
    def render_to_response(self, *args, **kwargs):
        data = Serializer().serialize(
            [self.object], fields=self.request.GET.get("fields"), **kwargs
        )
        return HttpResponse(data, content_type="application/json")
...
def handler404(request, exception):
    if "/api/" in request.path:
        return MyJsonResponse(
            context={"message": "Not found", "error": f"{exception}"}, status=404
        )  # some json
